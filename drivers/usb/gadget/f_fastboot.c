...
static void cb_flash(struct usb_ep *ep, struct usb_request *req)
{
	...
	if(lock_flashing){
		error("Prevents flash,need flashing unlock!\n");
		return;
	}
...
static void cb_flashing(struct usb_ep *ep, struct usb_request *req)
{
	...
	printf("cmd: %s",cmd);
	//strsep(&cmd, ":");
	if(!strcmp(cmd,"flashing lock")){
		lock_flashing = true;
	}
	else if(!strcmp(cmd,"flashing unlock")){
		lock_flashing = false;
	}
	printf("cmd: %s %s: at line %d\n",cmd,__func__,__LINE__);
}
