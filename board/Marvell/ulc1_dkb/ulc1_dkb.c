...
/* Get board serial number from emmc and check it. */
int emmc_get_file_from_ext4(char *data, int length, char *filename)
{
	char cmd[128];
	int ret = -1;
	...
	char *load_addr = (char *)(CONFIG_LOADADDR - 0x2000);
	...
	for (i = 1; i <= pnum; i++) {
		if (get_partition_info(dev_desc, i, &info))
			break;
		if(strncmp((const char *)info.name, CONFIG_FACTORY_PARTITION, sizeof(CONFIG_FACTORY_PARTITION)) == 0)
			break;
	}

	if(i > pnum){
		error("No partition \"%s\" is found\n", CONFIG_FACTORY_PARTITION);
		return ret;
	}

	/* copy boot image and uImage header to parse */
	sprintf(cmd, "ext4load mmc %d:%x %p %s", CONFIG_FASTBOOT_FLASH_MMC_DEV, i, \
			load_addr, filename);
	...
	if(!ret){
		if(data != NULL){
			...
			if(length < filesize)
				filesize = length;
			memcpy(data, load_addr, filesize);
		}
	}
...
int emmc_get_board_mac(char data0[], char data1[])
{
	/*struct mmc_cid cid;*/
	char csum[32+1];
	char *h = &csum;
	...
	*/
	...
	sha256_csum_wd(&(card->cid), sizeof(card->cid), csum, 0);
	sprintf(data0, "%02x:%02x:%02x:%02x:%02x:%02x", 0, 0x50, 0x43, h[0], h[1], h[2]);
	sprintf(data1, "%02x:%02x:%02x:%02x:%02x:%02x", 0, 0x50, 0x43, h[3], h[4], h[5]);
...
int misc_init_r(void)
{
	...
	*(unsigned int *)(RTC_BRN0) = 0;
	...
	#if 0
	if(emmc_get_file_from_ext4(NULL, 0, CONFIG_FACTPART_USE_NVM_SN) == 0){
		if(emmc_get_file_from_ext4(board_sn, sizeof(board_sn), CONFIG_FACTPART_BOARD_SN)){
			emmc_get_board_sn_from_cid(board_sn);
		}
	}else{
		emmc_get_board_sn_from_cid(board_sn);
	}
	if(emmc_get_file_from_ext4(NULL, 0, CONFIG_FACTPART_USE_NVM_MAC)){
		if(emmc_get_board_mac(board_wifi_mac, board_bluetooth_mac) == 0){
			remove_cmdline_param(cmdline, "androidboot.wifi.mac=");
			sprintf(cmdline + strlen(cmdline),
				" androidboot.wifi.mac=%s", board_wifi_mac);
			remove_cmdline_param(cmdline, "androidboot.bluetooth.mac=");
